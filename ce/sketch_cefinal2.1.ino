...
// 全局变量
int previousBrightness = -1; // 用于存储上一次亮度值
unsigned long lastUpdateTime = 0; // 上次更新的时间戳
const unsigned long updateInterval = 5000; // 更新间隔（毫秒）
...
void controlBrightness() {
  ...
  // 如果亮度变化超过阈值，或者超过强制刷新间隔，则发送更新
  if (abs(brightness - previousBrightness) > threshold || (currentTime - lastUpdateTime > updateInterval)) {
    previousBrightness = brightness; // 更新记录
    lastUpdateTime = currentTime;    // 更新上次刷新时间
